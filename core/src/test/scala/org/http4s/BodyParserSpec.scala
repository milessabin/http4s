...
//      case req => ( _ => xml(req.charset) { elem => Ok(elem.label) })
//    })
//
//    "parse the XML" in {
//      val resp = Await.result(server(RequestPrelude(), Spool.Cons("<html><h1>h1</h1></html>").map(s => BodyChunk(s))), 2 seconds)
//      resp.statusLine.code must_==(200)
//      resp.body must_==("html".getBytes)
//    }
//
//    "handle a parse failure" in {
//      val resp = Await.result(server(RequestPrelude(), Spool.Cons("This is not XML.").map(s => BodyChunk(s))), 2 seconds)
...
//      Await.result(server(req, Spool.Cons(BodyChunk(binData))), 2 seconds)
//    }
//
